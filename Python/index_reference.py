...
def text_deciphered(text, answer_keys):

    word_list = text.split(" ")
    
    for answers, abbrevs in answer_keys.items():
        for i in range(len(word_list)):
            if word_list[i] == abbrevs:
                word_list[i] = answers
    ...
    # for answers, abbrevs in answer_keys.items():
    #     text = text.replace(" "+ abbrevs, " "+ answers+ " ")
...
text_message = "Hey, wat r our plans for tn"
abbrevs = {'what': 'wat', 'are': 'r', 'tonight': 'tn', 'before': 'b4', 'see': 'c', 'you': 'u', 'okay': 'ok'}
