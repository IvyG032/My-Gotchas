...
def check_winner(tuples):
    ...
    for r in range(3):
        for c in range(7):
            if tuples[r][c] == tuples[r+1][c] == tuples[r+2][c] == tuples[r+3][c] and tuples[r][c] is not None:
                return tuples[r][c]

    #check for horizontal
    for r in range(6):
        for c in range(4):
            if tuples[r][c] == tuples[r][c+1] == tuples[r][c+2] == tuples[r][c+3] and tuples[r][c] is not None:
                return tuples[r][c]
            
    #check for diagonal 
    for r in range(3):
        for c in range(4):
            if tuples[r][c] == tuples[r+1][c+1] == tuples[r+2][c+2] == tuples[r+3][c+3] and tuples[r][c] is not None:
                return tuples[r][c]
    for r in range(3,6):
            for c in range(4):
                if tuples[r][c] == tuples[r-1][c+1] == tuples[r-2][c+2] == tuples[r-3][c+3] and tuples[r][c] is not None:
